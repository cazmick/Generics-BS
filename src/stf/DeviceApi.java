...
public class DeviceApi {
    ...
    public boolean connectDevice(String deviceSerial) {
        ...
        try {
            ...
            if (!present || !ready || using || owner) {
                ...
                if(owner && !ownerElement.getAsJsonObject().get("name").toString().equals("\"automation\"")){
                	LOGGER.severe("Stopping execution as device is busy. Current Owner is "+ownerElement.getAsJsonObject().get("name"));
                	System.exit(4);
                }
                return false;
       	 ...
       	 LOGGER.severe("Error!! Could not attach device.");
       	 return false;
       }
    }
    ...
    private boolean addDeviceToUser(String deviceSerial) {
        RequestBody requestBody = RequestBody.create(JSON, "{\"serial\": \"" + deviceSerial + "\", \"timeout\": 3600000}");
        Request request = new Request.Builder()
                .addHeader("Authorization", "Bearer " + stfService.getAuthToken())
    ...
    public boolean releaseDevice(String deviceSerial) {
        ...
        try {
            ...
            return true;
        	 ...
        	 LOGGER.severe("Error!! Could not release device.");
        	 return false;
        }
    }
}
